...
def trajektori_subjek(frames: list[dict[str, Any]], fps: float,
    ...
    f_awal, w_awal = _pilih_subjek(frames, klik, fps, scene_start)
    if f_awal < 0:
        return {}
...
async def simpan_manual_track(clip_id: str, user_id: str,
    ...
    if not frames and render_mod is not None and source_url_for is not None:
        # layout_frames belum ada → analisis penuh SEKALI (via run_sync
        # supaya tidak memblokir event loop), lalu simpan untuk selamanya.
        ...
        ct = {**ct, "layout_frames": frames, "analysis_fps": fps,
              "src_w": src_w}

    t = trajektori_subjek(frames, fps, klik, scene_start, scene_end, src_w)
    if not t:
